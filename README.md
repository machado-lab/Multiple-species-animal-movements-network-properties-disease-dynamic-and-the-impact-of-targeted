...
nico_sier_granjas_random_fun <- function(sim.number = NA, 
                                         ...
                                         events, # eventos programados en formato para siminf
                                         infectados.ini, # codigos propiredades infectadas al comienzo de la sim 
                                         granjas, # net$correspondece com a populacoe 
                                         Specie.to.select, # especie para sembrar infecciones 
                                         granjas.especie,# cadastro granjas repitiendo granjas si tiene mas de una especie
                                         tspan # period of the simulation 
) {
  
  # asignamos objetos para la funcion
  ...
  #recuperamos las id de las granjas por especie 
  ...
  #cada vez que roda el modelo seleciona granjas diferentes 
  ...
  # banco de cadastro por compartimentos y  metapoblaciones cada linea = una granja 
  
  u0 <- data.frame(S = granjas$pop_total,
                   I = rep(0, nrow(granjas)),
                   R = rep(0, nrow(granjas)))
  
  u0$I[infectados.ini.random] <- round(u0$S[infectados.ini.random] * p_anim_inf)
  ...
  #seleccionamos las granjas de acuerdo al paramtero de SNA 
  ...
  # propriedades intervenidas no mandan ni reciben animales 
  node.to.remove <- step_rm # nodes  a  remover 
  ...
  model1 <- SIR(u0 = u01,                  # initial status of farms 
                tspan = tspan,             # periodo de la simulacion 
  ...
  # run the model 
  result <- run(model = model1, threads = 4)  ################################################## aqui o numero de nucleos 
  # contando las granjas infectadas por dia
